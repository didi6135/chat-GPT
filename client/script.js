...
function generateUniqueId() {
  ...
  const number2 = randomNumber.toString(16)

  return `id-${timestamp}-${number2}`
}
...
const handleSubmit = async (e) => {
  ...
  chetConteiner.scrollTop = chetConteiner.scrollHeight

  const messageDiv = document.getElementById(uniqueId)

  loader(messageDiv)

  const response = await fetch('https://chat-gpt-vtha.onrender.com' , {
    method: 'POST',
    headers: {
  ...
  if(response.ok) {
    ...
    const paraseData = data.bot.trim()
